...
export function SchedulesWorkspace({ token }: { token: string }) {
  ...
  const selectedSchedule = useMemo(() => {
    ...
    return schedules.find((schedule) => schedule.id === selectedScheduleId) ?? schedules[0] ?? null
  ...
  const refreshSchedules = async () => {
    await Promise.all([
      queryClient.invalidateQueries({ queryKey: miboQueryKeys.schedules(token) }),
      historySchedule?.id
        ? queryClient.invalidateQueries({ queryKey: miboQueryKeys.scheduleHistory(token, historySchedule.id) })
        : Promise.resolve(),
    ])
  }
  ...
  const toggleMutation = useMutation({
    mutationFn: (schedule: Schedule) => api.toggleSchedule(schedule.id, !schedule.enabled),
    onSuccess: async () => {
      await refreshSchedules()
      ...
      <div className="mx-auto max-w-6xl space-y-4">
        <div className="flex flex-col gap-4 rounded-[1.75rem] border border-border/60 bg-card/80 p-5 shadow-sm backdrop-blur-sm sm:flex-row sm:items-end sm:justify-between">
          <div className="space-y-2">
            <Badge variant="outline" className="border-border/60 bg-background/70">Phase 10</Badge>
            <h1 className="text-3xl font-semibold tracking-tight">计划任务工作台</h1>
            <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
              独立管理 recurring maintenance schedules，直接查看 enabled 状态、next run、latest result，并在详情层展开最近运行历史。
            </p>
          </div>

          <Button asChild variant="outline" className="border-border/60 bg-background/70">
            <Link to="/settings">
        ...
        <Alert>
          ...
          <AlertDescription>
            当前工作台保持 schedule-first 视角；设置页只负责摘要与跳转，完整 CRUD / run-now / history 均留在此处。
          </AlertDescription>
        </Alert>
        ...
        <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
          <Card className="rounded-[1.5rem] border-border/60 bg-card/80 py-0 shadow-sm">
            <CardHeader className="px-5 py-5">
              ...
              <CardDescription>主列表直接呈现任务类型、目标范围、频率模板、启停状态和最新结果。</CardDescription>
            </CardHeader>
            <Separator className="bg-border" />
            <CardContent className="space-y-4 px-5 py-5">
              {schedulesQuery.isLoading ? (
                <div className="rounded-[1.1rem] border border-border/60 bg-background/60 px-4 py-6 text-sm text-muted-foreground">正在加载计划任务…</div>
          ...
          <Card className="rounded-[1.5rem] border-border/60 bg-card/80 py-0 shadow-sm">
            <CardHeader className="px-5 py-5">
              ...
              <CardDescription>详情层继续承接运行历史与后续执行反馈。</CardDescription>
            </CardHeader>
            <Separator className="bg-border" />
            <CardContent className="space-y-4 px-5 py-5 text-sm text-muted-foreground">
              {selectedSchedule ? (
                <>
                  <DetailRow label="任务类型" value={formatKind(selectedSchedule.kind)} />
                  <DetailRow label="目标范围" value={formatScope(selectedSchedule.scope_kind, selectedSchedule.library_id)} />
                  <DetailRow label="频率模板" value={formatFrequency(selectedSchedule.frequency)} />
                  <DetailRow label="下次运行" value={formatDateTime(selectedSchedule.next_run_at)} />
                  <DetailRow label="最近结果" value={formatLatestResult(selectedSchedule)} />
                  <div className="rounded-[1.1rem] border border-border/60 bg-background/60 px-4 py-3">
                    <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">详情层入口</div>
                    <div className="mt-1 text-sm text-foreground">查看最近运行历史并跟踪 queued / running / completed / failed 状态。</div>
                  </div>
                </>
              ) : (
                <div className="rounded-[1.1rem] border border-dashed border-border/60 px-4 py-6 text-center">选择一条计划任务后查看更多信息。</div>
              )}

              <div className="rounded-[1.1rem] border border-border/60 bg-background/60 px-4 py-3">
                <div className="text-xs uppercase tracking-[0.16em] text-muted-foreground">数据来源</div>
                <div className="mt-1 text-sm text-foreground">列表、详情和历史都通过 typed API + TanStack Query 管理，不解析 payload_json，也不回退到 /api/v1/jobs 重建语义。</div>
              </div>
            </CardContent>
          </Card>
        </div>
