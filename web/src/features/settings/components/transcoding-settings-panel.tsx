  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Tabs
        value={activeTab}
        onValueChange={(value) =>
          setActiveTab(value as 'transcoding' | 'tone-mapping')
        }
        className="space-y-4"
      >
          ...
          <CardHeader className="gap-4 px-5 py-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              ...
              <TabsList className="grid w-full grid-cols-2 sm:w-fit">
                <TabsTrigger value="transcoding">转码</TabsTrigger>
                <TabsTrigger value="tone-mapping">色调映射</TabsTrigger>
              </TabsList>
            </div>
          </CardHeader>
        </Card>

        <TabsContent value="transcoding" className="mt-0">
          <TranscodingTab draft={draft} updateDraft={updateDraft} />
        </TabsContent>

        <TabsContent value="tone-mapping" className="mt-0">
