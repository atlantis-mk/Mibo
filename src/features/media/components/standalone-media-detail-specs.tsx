...
function PeopleRail({
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className='relative -mr-6 sm:-mr-8 lg:-mr-10'>
        <Swiper
          modules={[FreeMode]}
          freeMode
          simulateTouch
...
export function SpecsSection({
  ...
  const resourceRows: [string, string][] = resourceChoices.map((resource) => [
    ...
    [
      formatComputedResourceVariantLabel(resource, resourceChoices),
...
function TrackChoiceSummary({
  ...
  const audioCount = audioStreams?.length ?? 0
  const subtitleCount = subtitleStreams?.length ?? 0
  ...
  return (
    <div className='flex flex-wrap gap-2'>
      ...
      >
        音轨：
        {formatTrackChoiceAudioSummary(defaultAudio, audioCount)}
      </Button>
      <Button
        variant='outline'
        className='rounded-full border-border/50 bg-background/75'
      ...
      >
        字幕：
        {formatTrackChoiceSubtitleSummary(defaultSubtitle, subtitleCount)}
      </Button>
    </div>
  )
}

function formatTrackChoiceAudioSummary(
  defaultAudio: CatalogMediaStreamSummary | undefined,
  audioCount: number
) {
  if (!defaultAudio) return '不可用'
  const defaultLabel =
    [
      formatStreamLanguage(defaultAudio.language),
      defaultAudio.title || formatCodecLabel(defaultAudio.codec),
    ]
      .filter(Boolean)
      .join(' · ') || '默认'
  return audioCount > 1
    ? `多音轨 ${audioCount} 条 · ${defaultLabel}`
    : defaultLabel
}

function formatTrackChoiceSubtitleSummary(
  defaultSubtitle: CatalogMediaStreamSummary | undefined,
  subtitleCount: number
) {
  if (!defaultSubtitle) {
    return subtitleCount > 0 ? `${subtitleCount} 条可用` : '关闭 / 不可用'
  }
  const defaultLabel =
    [
      formatStreamLanguage(defaultSubtitle.language),
      defaultSubtitle.title || formatCodecLabel(defaultSubtitle.codec),
    ]
      .filter(Boolean)
      .join(' · ') || '默认'
  return subtitleCount > 1
    ? `${subtitleCount} 条 · ${defaultLabel}`
    : defaultLabel
}
