...
import { ArrowRightIcon, LoaderCircleIcon, PlayIcon } from 'lucide-react'
import { type Library, type Schedule } from '@/lib/mibo-api'
...
export function ScheduleList({
  ...
  if (schedules.length === 0 && libraries.length === 0) {
    return (
      <div className='rounded-xl border border-dashed border-border/70 bg-card/50 px-6 py-10 text-center'>
        ...
        <p className='mx-auto mt-2 max-w-xl text-sm leading-6 text-muted-foreground'>
          创建扫描或链接检查任务后，这里会按维护类别显示上次运行时间、耗时和手动执行入口。
        </p>
        <div className='mt-5'>
                ...
                >
                  <div className='min-w-0 space-y-2'>
                    ...
                    <p className='max-w-3xl text-sm leading-6 text-muted-foreground'>
                      {formatDescription(schedule, libraryNames)}
                    </p>
        ...
        <section className='space-y-2'>
          ...
          <div className='overflow-hidden rounded-xl border border-border/60 bg-card/40'>
            {libraries.map((library, index) => {
              const scanning =
                isScanningLibrary && scanningLibraryId === library.id
              const scheduled = Boolean(
                schedules.find(
                  (schedule) =>
                    schedule.kind === 'scan' &&
                    schedule.scope_kind === 'library' &&
                    schedule.library_id === library.id
                )
              )
              ...
              return (
                ...
                >
                  <div className='min-w-0 space-y-2'>
                    ...
                    <div className='flex flex-wrap gap-x-5 gap-y-1 text-sm text-muted-foreground'>
                      <span>状态：{formatLibraryStatus(library.status)}</span>
                      <span>路径：{library.root_path}</span>
                    </div>

                    <p className='max-w-3xl text-sm leading-6 text-muted-foreground'>
                      立即扫描该媒体库的新增与变更文件；如需定时执行，可创建
                      scope 为该媒体库的扫描计划。
                    </p>
                  </div>
                  ...
                  <Button
                    ...
                    variant='outline'
                    size='icon'
                    disabled={scanning || !library.scanner_enabled}
                  ...
                  >
                    ...
                    <span className='sr-only'>立即扫描 {library.name}</span>
                  </Button>
                </article>
              )
            })}
        ...
        <div className='overflow-hidden rounded-xl border border-border/60 bg-card/40'>
          <article className='grid gap-4 px-4 py-4 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-5'>
            <div className='min-w-0 space-y-2'>
              ...
              <p className='max-w-3xl text-sm leading-6 text-muted-foreground'>
                打开元数据治理工作台，对需要处理的条目执行重新匹配、刷新详情和修正匹配结果。
              </p>
            </div>
            ...
            <Button asChild type='button' variant='outline' size='icon'>
              <Link to='/settings/metadata'>
                ...
                <span className='sr-only'>打开元数据治理工作台</span>
              </Link>
            </Button>
          </article>
        </div>
...
function formatScope(
  ...
  return scope === 'library'
    ? libraryId
      ? (libraryNames?.get(libraryId) ?? `媒体库 #${libraryId}`)
      : '单媒体库'
    : '全局范围'
}
