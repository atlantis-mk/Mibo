...
export function TranscodingSettingsPanel({
  ...
  activeTab: 'transcoding' | 'tone-mapping'
}) {
  ...
  return (
    <>
      <form
        id='transcoding-settings-form'
        onSubmit={handleSubmit}
        className='space-y-4 pb-20'
      >
        <div>
          {activeTab === 'transcoding' ? (
            <TranscodingTab draft={draft} updateDraft={updateDraft} />
          ) : (
            <ToneMappingTab draft={draft} updateDraft={updateDraft} />
          )}
        </div>
      </form>
      ...
      {createPortal(
        <div className='fixed right-6 bottom-6 z-[100] flex justify-end'>
          <Button type='submit' form='transcoding-settings-form'>
            保存配置
          </Button>
        </div>,
        document.body
...
function TranscodingTab({
  ...
  draft: TranscodingSettingsForm
  updateDraft: <Value extends keyof TranscodingSettingsForm>(
    key: Value,
    value: TranscodingSettingsForm[Value]
  ) => void
}) {
      ...
      >
        <div className='space-y-3'>
          {softwareEncoders.map((encoder) => (
            <div
              key={encoder.name}
              className='flex items-center justify-between gap-4 rounded-[1.15rem] border border-border bg-background px-4 py-3'
            >
              ...
              <Button
                ...
                onClick={() => toast.info(`${encoder.name} 参数设置稍后接入`)}
              >
          ...
          <Field>
            ...
            <div className='flex gap-2'>
              ...
              <Button
                ...
                size='icon'
                className='shrink-0'
                onClick={() => toast.info('路径选择器稍后接入服务端目录浏览')}
              >
...
function ToneMappingTab({
  ...
  return (
    ...
    >
      <div className='flex items-start gap-3 rounded-[1.15rem] border border-border bg-muted/40 px-4 py-3 text-sm leading-6 text-muted-foreground'>
...
function TranscodingSwitchField({
  ...
  return (
    <Field
      ...
      className='items-start rounded-[1.25rem] border border-border bg-background p-3.5'
    >
      <Switch
        checked={checked}
