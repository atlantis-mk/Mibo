...
export function UserManagementPanel() {
  ...
  const updateUserMutation = useMutation({
    mutationFn: async () => {
      ...
      if (editDraft.password.trim()) {
        input.password = editDraft.password
      }
      if (editDraft.pin.trim()) {
        input.pin = editDraft.pin
      }

      return createAuthedMiboApi(token).updateAdminUser(editingUser.id, input)
    },
    onSuccess: async (updatedUser) => {
      toast.success(`用户 ${updatedUser.username} 已更新。`)
      setEditingUser(null)
      setEditDraft(EMPTY_EDIT_USER_FORM)
      await queryClient.invalidateQueries({
        queryKey: miboQueryKeys.adminUsers(queryToken),
      })
  ...
  function openEditDialog(user: SettingsUser) {
    setEditingUser(user)
    setEditDraft({
                ...
                {users.map((user) => {
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        'group rounded-[1.25rem] border bg-background/75 p-4 text-left shadow-sm transition-all hover:-translate-y-0.5 hover:border-emerald-500/50 hover:shadow-md focus-visible:ring-3 focus-visible:ring-emerald-500/25 focus-visible:outline-none',
                        selected
        ...
        <form className='grid gap-4' onSubmit={onSubmit}>
          ...
          <label className='space-y-1.5 text-sm'>
            ...
            <Input
              ...
              placeholder='4 位数字'
              onChange={(event) =>
                onChange({ ...draft, pin: event.target.value })
              }
            />
          </label>
          <label className='space-y-1.5 text-sm'>
            <span className='text-muted-foreground'>头像 URL</span>
            <Input
              ...
              onChange={(event) =>
                onChange({ ...draft, avatar_url: event.target.value })
              }
            />
          </label>
          <label className='space-y-1.5 text-sm'>
...
function EditUserDialog({
  ...
  draft: EditUserFormState
  isSubmitting: boolean
  onOpenChange: (open: boolean) => void
  onSubmit: (event: FormEvent<HTMLFormElement>) => void
  onChange: (draft: EditUserFormState) => void
}) {
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='sm:max-w-lg'>
        <DialogHeader>
          ...
          <DialogDescription>
            {user ? `修改 ${user.username} 的账号信息。` : '修改用户信息。'}
          </DialogDescription>
        </DialogHeader>
        <form className='grid gap-4' onSubmit={onSubmit}>
          <label className='space-y-1.5 text-sm'>
          ...
          <label className='space-y-1.5 text-sm'>
            ...
            <Input
              ...
              pattern='[0-9]*'
              autoComplete='new-password'
              placeholder={user?.has_pin ? '留空表示不修改' : '4 位数字'}
              onChange={(event) =>
                onChange({ ...draft, pin: event.target.value })
              }
            />
          ...
          <label className='space-y-1.5 text-sm'>
            ...
            <NativeSelect
              ...
              onChange={(event) =>
                onChange({
                  ...draft,
                  role: event.target.value as EditUserFormState['role'],
                })
              }
            >
              <option value='user'>普通用户</option>
        ...
        {user ? (
          <>
            ...
            <DetailRow
              ...
              label='角色'
              value={formatRoles(user.roles ?? (user.role ? [user.role] : []))}
            />
            <DetailRow
              icon={Clock3Icon}
              label='最近活动'
