...
export const Route = createFileRoute('/_authenticated/library-browser')({
  component: LibraryBrowserRoute,
  validateSearch: (search: Record<string, unknown>) => ({
    ...
    ...(normalizeLibraryIdSearch(search.libraryId) !== undefined
      ...
      : {}),
    ...(normalizeLibraryPathSearch(search.path) !== undefined
      ? { path: normalizeLibraryPathSearch(search.path) }
      : {}),
...
function LibraryBrowserRoute() {
  const navigate = useNavigate({ from: '/library-browser' })
  ...
  const pageSize = search.pageSize ?? DEFAULT_LIBRARY_BROWSER_PAGE_SIZE
  const scrollStorageKey = `mibo:library-browser-scroll:${locationHref}`
...
function normalizeLibraryPathSearch(value: unknown) {
...
function normalizeLibraryPageSizeSearch(value: unknown) {
  ...
  return Number.isFinite(parsed) && isLibraryBrowserPageSize(parsed)
    ? parsed
    : undefined
}
