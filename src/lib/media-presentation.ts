...
export type MediaDetailPresentation = {
  ...
  image_candidates: CatalogItemDetail['image_candidates']
...
export function parseMediaDetailView(
  value: unknown
): MediaDetailView | undefined {
  return value === 'series' ? 'series' : undefined
}
...
export function metadataItemDetailToPresentation(
  ...
  const backdropImages = Array.from(
    new Set(
      [
        ...(item.selected_images ?? [])
          .filter(
            (image) =>
              image.image_type === 'backdrop' || image.image_type === 'still'
          )
          .map((image) => image.url),
        ...(item.image_candidates ?? [])
          ...
          .map((image) => image.url),
        ...(episodeContext?.series?.selected_images ?? [])
          .filter((image) => image.image_type === 'backdrop')
          .map((image) => image.url),
      ].filter(Boolean)
    )
  )
