...
The system SHALL execute movie search through MetaTube HTTP endpoints and normalize returned MetaTube movie search results into Mibo catalog search candidates without pretending they are TMDB results.
...
- **THEN** the system MUST call the configured MetaTube server search endpoint and convert successful results into provider `metatube` candidates
...
The system SHALL fetch MetaTube movie detail over HTTP and apply normalized metadata to catalog fields, image candidates, people, external identities, and source evidence according to existing catalog governance rules.
...
- **THEN** the system MUST apply supported values to unlocked catalog fields and related catalog records using the same governance protections as other metadata providers
...
The system SHALL refetch catalog metadata from MetaTube when the selected provider identity and library strategy identify a usable MetaTube provider instance.
...
- **THEN** refetch MUST call MetaTube detail for the stored upstream provider and ID and update metadata evidence without requiring a new search
...
- **THEN** refetch MUST fail with a clear error instead of using a TMDB identity or guessing an upstream ID
