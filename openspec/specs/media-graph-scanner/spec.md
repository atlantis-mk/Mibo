...
## Purpose
TBD - created by archiving change media-graph-scanner-architecture. Update Purpose after archive.
## Requirements
### Requirement: Scanner separates inventory facts from media decisions
The system SHALL collect storage facts independently from catalog media classification decisions.

#### Scenario: Video file is scanned
- **WHEN** the scanner encounters a supported video file
...
The system SHALL group scanned directories, files, and sidecars into media graph candidates before writing catalog items for movie and TV libraries.

#### Scenario: Directory contains multiple episode-like videos
- **WHEN** a TV library directory contains multiple files that resolve to episode slots
...
- **WHEN** a movie library folder contains multiple plausible main video files for the same work
...
- **WHEN** a resolver groups a flat TV folder into a series candidate
...
The system SHALL reconcile scanner-created catalog items using durable identities that are independent of display title.
...
The system SHALL project TV media graph decisions into durable catalog series, season, and episode hierarchy rows.

#### Scenario: Standard TV folder is scanned
- **WHEN** the scanner processes `Series/Season 1/Series - S01E01.mkv`
...
- **WHEN** the scanner processes a TV directory containing `S02E01`, `S02E02`, and `第03集` style files for the same work
...
The system SHALL project movie folder decisions into one movie catalog item with separate assets for main files, versions, and extras.

#### Scenario: Movie has two quality versions
- **WHEN** a movie folder contains `Movie.1080p.mkv` and `Movie.2160p.mkv`
...
- **WHEN** a movie folder contains a main movie file plus trailer or behind-the-scenes videos
...
- **WHEN** the scanner processes `Show.S01E01-E02.mkv`
...
The system SHALL keep media graph and resolver concepts generic enough to support future audio, document, and photo media types.
