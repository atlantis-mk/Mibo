...
- **THEN** the system SHALL use path, filename, extension, sidecar-name, already-listed object metadata, structured filename signals, cached directory summary evidence, path-tree assignments, group-level evidence, and supported sidecar/external identity hints without running ffprobe, content hashing, external metadata searches, artwork downloads, or additional recursive source analysis in the fast path
...
#### Scenario: Weak movie fallback is held back
- **WHEN** classification only has weak movie fallback evidence such as normalized title and year without strong group-level confirmation
- **THEN** the system MUST keep the item reviewable and inventory-visible instead of directly creating final movie metadata
