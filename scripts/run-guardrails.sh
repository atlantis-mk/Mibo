...
(cd "$ROOT_DIR/mibo-media-server" && go test ./internal/httpapi -run 'Test(QueueLibraryScanBehavior|CatalogPlaybackBehavior|GovernanceBehaviorUpdatesFieldAndVisibility|Health)')

printf '\n[guardrails] backend library end-to-end guardrails\n'
(cd "$ROOT_DIR/mibo-media-server" && go test ./internal/library -run 'Test(ListenerRefreshToScanToProjectionEndToEnd|TargetedRefreshWorkflowKeepsScopedRootInProjectionTask|GovernanceVisibilityRemovesHomeProjectionAfterRefresh|MaterializedPlaybackStillResolvesAfterProjectionRefresh|QueueLibraryWorkflow|RunWorkflowScanLibraryPath|QueueLibraryScanWithReason)')

printf '\n[guardrails] backend listener\n'
(cd "$ROOT_DIR/mibo-media-server" && go test ./internal/listener)

printf '\n[guardrails] backend playback\n'
(cd "$ROOT_DIR/mibo-media-server" && go test ./internal/playback)

printf '\n[guardrails] backend health\n'
(cd "$ROOT_DIR/mibo-media-server" && go test ./internal/health)
