...
- Legacy `/api/v1/media-items/*` and `/api/v1/media-files/*` routes are retired; use `/api/v1/items/*`, `/api/v1/items/{id}/resources`, and `/api/v1/inventory-files/*` instead.
...
- Governance repair endpoints now include metadata/resource correction paths: `POST /api/v1/items/{id}/governance/resources/{resource_id}/links`, `PATCH /api/v1/items/{id}/governance/resources/{resource_id}/links/{target_metadata_item_id}`, `DELETE /api/v1/items/{id}/governance/resources/{resource_id}/links/{target_metadata_item_id}`, `POST /api/v1/items/{id}/governance/metadata-merge`, `POST /api/v1/items/{id}/governance/metadata-split`, and `PUT /api/v1/items/{id}/governance/projection-visibility`.
- The resource-first catalog is development-reset oriented. Before validating a major schema cutover locally, clear old `mibo-media-server/data/` state or point the server at a fresh SQLite database, then rescan demo media using `MIBO_LOCAL_ROOT_PATH=/Users/atlan/Desktop/IdeaProjects/Mibo/demo-media`.
- After a resource-first reset/rescan, manually verify browse, detail, episode shelves, multi-version playback, progress/continue watching, favorites, search, home, governance corrections, scan exclusion, and reprobe before treating a catalog read-model cutover as complete.
- Do not stop AutoMigrating legacy catalog read-model tables until `CatalogItem`, `CatalogSearchDocument`, `ItemRollup`, `MediaAsset`, `AssetItem`, `AssetFile`, and `UserItemData` are no longer referenced by product code or required tests.
